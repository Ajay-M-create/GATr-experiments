...
class SymmetryGATrWrapper(BaseWrapper):
    """Wraps around GATr for the symmetry prediction experiment.
    ...
    def extract_from_ga(self, multivector, scalars):
        """Extracts predicted symmetries from the GATr multivector outputs.
        ...
        outputs : torch.Tensor
            Predicted symmetries, shape (batchsize, 1).
        other : torch.Tensor
            Regularization terms.
        """

        # Assume the first grade is related to symmetry
        scalar_output = extract_scalar(multivector[:, :, 0, :])  # (batchsize, num_points)

        # Aggregate the scalar outputs across all points to get a single symmetry prediction per sample
...
class SymmetryTransformerWrapper(nn.Module):
    """Wraps around a baseline Transformer for the symmetry prediction experiment.
