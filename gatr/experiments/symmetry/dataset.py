...
class SymmetryDataset(torch.utils.data.Dataset):
    """Symmetry prediction dataset.

    Loads data generated with generate_symmetry_dataset.py from disk.
    ...
    def _load_data(filename, subsample=None, keep_extra=False):
        ...
        points, symmetries = npz["points"], npz["symmetries"]

        # Convert to tensors
        points = torch.from_numpy(points).to(torch.float32)  # (num_samples, num_points, 3)
        symmetries = torch.from_numpy(symmetries).to(torch.float32).unsqueeze(1)  # (num_samples, 1)
        ...
        if subsample is not None and subsample < 1.0:
            ...
            symmetries = symmetries[:n_keep]
            if extra is not None:
                extra = extra[:n_keep]

        return points, symmetries, extra
