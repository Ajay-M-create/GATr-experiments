...
class VolumeDataset(torch.utils.data.Dataset):
    """Volume prediction dataset.

    Loads data generated with generate_volume_dataset.py from disk.
    ...
    def _load_data(filename, subsample=None, keep_extra=False):
        ...
        points, volumes = npz["points"], npz["volumes"]

        # Convert to tensors
        points = torch.from_numpy(points).to(torch.float32)  # (num_samples, 5, 3)
        volumes = torch.from_numpy(volumes).to(torch.float32).unsqueeze(1)  # (num_samples, 1)
        ...
        if subsample is not None and subsample < 1.0:
            ...
            volumes = volumes[:n_keep]
            if extra is not None:
                extra = extra[:n_keep]

        return points, volumes, extra