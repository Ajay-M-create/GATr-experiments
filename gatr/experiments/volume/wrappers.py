...
class VolumeGATrWrapper(BaseWrapper):
    """Wraps around GATr for the volume prediction experiment.
    ...
    def extract_from_ga(self, multivector, scalars):
        """Extracts predicted volumes from the GATr multivector outputs.
        ...
        outputs : torch.Tensor
            Predicted volumes, shape (batchsize, 1).
        other : torch.Tensor
            Regularization terms.
        """

        # Assume the first grade is related to volume
        scalar_output = extract_scalar(multivector[:, :, 0, :])  # (batchsize, num_points)

        # Aggregate the scalar outputs across all points to get a single volume prediction per sample
...
class VolumeTransformerWrapper(nn.Module):
    """Wraps around a baseline Transformer for the volume prediction experiment.
